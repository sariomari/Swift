...
def update_driver_zone(driver_id, latitude, longitude):
    ...
    new_driver_zone = get_zone_from_lat_long(latitude=latitude, longitude=longitude)
    ...
    new_driver_obj = Driver.objects.create(
        driver_id=driver_id,
        latitude=latitude,
        longitude=longitude,
...
def sendOrderNow(userID, user_location, store_location, items):
    ...
    for driver_id in drivers_in_zone:
        assign_task_to_driver(driver_id, user_lat, user_long, store_lat, store_long, items)
