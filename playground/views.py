...
from driver_utils import update_driver_zone
...
def update_location(request):
    if request.method == 'POST':
        ...
        driver_object = Driver.objects.create(
            driver_id=driver_id,
            latitude=latitude,
            longitude=longitude
        )
        if driver_object.is_valid():
            update_driver_zone(driver_id=driver_id, latitude=latitude, longitude=longitude)
            driver_object.save()
